...
case class Post(id: String, createdBy: String, text: String, createdAt: DateTime, userOpt: Option[User] = None)
...
object Post extends ORMapperWithStringId[Post] {
  ...
  val userRef = belongsToWithFk(User, "created_by", (p, u) => p.copy(userOpt = u))

  override def extract(rs: WrappedResultSet, n: scalikejdbc.ResultName[Post]): Post = autoConstruct(rs, n, "userOpt")
